...
void RpcProvider::Run()
{
    ...
    for(auto &sp : m_serviceMap)
    {
        ...
        {
            // /serive_name/method_name /UserServiceRpc/Login
                            ...
                            muduo::Timestamp)
{
    ...
    recv_buf.copy((char*)&header_size,4,0);//从0开始，读取四个字节的内容
    ...
    {
        // std::cout<<"request parse erro, content: "<<args_str<<std::endl;
        LOG_INFO("request parse erro, content: %s",args_str.c_str());
        return;
    }
    ...
    */
    //给相应的method方法的调用，绑定一个Closure的回调函数
...
void RpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr&conn, google::protobuf::Message*response)
{
    ...
    else
    {
        ...
        LOG_INFO("serialize reponse_str error! ");
    }
    conn->shutdown();//模拟http的短链接服务，由于rpcprovider主动断开连接
}